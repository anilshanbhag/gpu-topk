...
template cudaError_t bitonicTopK(float *d_keys_in, unsigned int num_items, unsigned int k, float *d_keys_out,
    CachingDeviceAllocator&  g_allocator);

template cudaError_t bitonicTopK(uint *d_keys_in, unsigned int num_items, unsigned int k, uint *d_keys_out,
    CachingDeviceAllocator&  g_allocator);
